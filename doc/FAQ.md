...
## Why is VSync disabled by default, and when should I enable it?
VSync is a frame timing mode where the output is synchronized with the monitor's refresh rate to eliminate tearing. Enabling VSync limits the FPS to the monitor's refresh rate (usually 60Hz) and forces the renderer to wait for the next vertical refresh. In windowed mode, running VSync through the window manager often causes minor stutters, even if the FPS is stable. Small drops below 60 FPS can make this effect even more noticeable. For this reason, Dagon disables VSync by default. As a general rule, it should only be enabled if you are experiencing screen tearing, and it only makes sense in exclusive fullscreen mode.
