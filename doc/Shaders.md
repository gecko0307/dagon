...
Shaders provide virtually unlimited possibilities for fine-tuning your game's visuals. From custom BRDFs to NPR and procedural textures, Dagon lets you do anything. The only drawback of using custom shaders on objects is that this completely overrides Dagon's built-in rendering for those objects. You'll need to implement basic effects such as lighting and environment mapping yourself.
...
```
SimpleShader simpleShader = New!SimpleShader(assetManager);
