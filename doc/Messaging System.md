...
- **Address**. A string that identifies a message/task event receiver (`Endpoint` or `EventListener`). A reserved empty address (empty string) is a special case: it means that the message is forwarded to all receivers. For tasks, it means that the sender does not care which endpoint should process it; the message broker is free to choose any.
