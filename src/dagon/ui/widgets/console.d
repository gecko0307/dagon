...
module dagon.ui.widgets.console;
...
class Console: UIWidget
{
   ...
   public:
    this(UIManager ui, UIWidget parent)
    {
        ...
        view.font = ui.fontManager.monospace;
        view.textColor = textColor;
        view.paddingBottom = 0;
        
        textCursor = ui.addElement(entity);
        textCursor.position.x = view.paddingLeft;
        textCursor.position.y = view.paddingTop;
    ...
    protected void encode()
    {
        ...
        foreach(i, dchar c; input)
        {
            ...
            if (numBytes > 0)
            {
                encodedInput ~= cast(string)encodeBuffer[0..numBytes];
            }
        }
        
        view.text = encodedInput.toString;
    ...
    protected void calcCursorPosition()
    {
        ...
        textCursor.position.x = view.paddingLeft + pos.x;
    ...
    override void onTextInput(dchar code)
    {
        ...
        if (input.length == 0 || textCursorPosition == input.length)
            ...
            input.insertKey(textCursorPosition, code);
        textCursorPosition++;
        encode();
    }
    ...
    override void onKeyDown(int key)
    {
        ...
        if (key == KEY_BACKSPACE)
        {
            ...
            {
                input.removeKey(textCursorPosition - 1);
                textCursorPosition--;
                encode();
            }
        }
        ...
        {
            evaluate(encodedInput.toString);
            clear();
        }
        ...
        {
            if (textCursorPosition < input.length)
            {
                textCursorPosition++;
                calcCursorPosition();
            }
