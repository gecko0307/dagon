...
TextureBuffer downloadTexture(Texture texture)
{
    ...
    if (tb.format.target == GL_TEXTURE_CUBE_MAP)
    {
        ...
        foreach(face; EnumMembers!CubeFace)
        {
            ...
            for (uint level = 0; level < tb.mipLevels; ++level)
            {
                ...
                {
                    uint imageSize =
                        ((cast(uint)mipWidth + 3) / 4) *
                        ((cast(uint)mipHeight + 3) / 4) *
                        tb.format.blockSize;
                ...
                {
                    bufferSize += mipWidth * mipHeight * tb.format.pixelSize;
                }
            }
        }
                ...
                if (tb.format.blockSize > 0)
                {
                    ...
                    if (imageSize > 0)
                    {
                        glGetCompressedTexImage(
                            faceTarget,
                            cast(GLint)level,
                            cast(void*)(data.ptr + offset));
                        offset += imageSize;
                    }
                }
                else
                {
                    uint size = mipWidth * mipHeight * tb.format.pixelSize;
    ...
    else // 1D, 2D, or 3D texture
    {
        for (uint level = 0; level < tb.mipLevels; ++level)
        {
            GLint mipWidth = 0, mipHeight = 0, mipDepth = 0;
            glGetTexLevelParameteriv(tb.format.target, cast(GLint)level, GL_TEXTURE_WIDTH, &mipWidth);
            glGetTexLevelParameteriv(tb.format.target, cast(GLint)level, GL_TEXTURE_HEIGHT, &mipHeight);
            glGetTexLevelParameteriv(tb.format.target, cast(GLint)level, GL_TEXTURE_DEPTH, &mipDepth);
            ...
            {
                bufferSize += mipWidth * mipHeight * mipDepth * tb.format.pixelSize;
            }
        }
        ...
        for (uint level = 0; level < tb.mipLevels; ++level)
        {
            ...
            {
                uint imageSize = mipWidth * mipHeight * mipDepth * tb.format.pixelSize;
